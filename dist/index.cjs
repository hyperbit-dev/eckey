...
var ECKey = class {
	...
	*/
	get publicKey() {
		if (!this._publicKey) this._publicKey = _noble_secp256k1.getPublicKey(this.key, this._compressed);
	...
	*/
	get pubKeyHash() {
		if (!this._pubKeyHash) {
			const sha = (0, _noble_hashes_sha2_js.sha256)(this.publicKey);
			this._pubKeyHash = (0, node_crypto.createHash)("ripemd160").update(Buffer.from(sha)).digest();
		}
	...
	*/
	get publicHash() {
		if (!this._publicHash) {
			const sha = (0, _noble_hashes_sha2_js.sha256)(this.publicKey);
			this._publicHash = (0, node_crypto.createHash)("ripemd160").update(Buffer.from(sha)).digest();
		}
	...
	*/
	sign(hash) {
		...
		return _noble_secp256k1.sign(hash, this.key).toCompactRawBytes();
	}
	/**
